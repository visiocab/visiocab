...
class Layout
{
	...
	public function show($view, $data = null)
	{
		...
		if($data != null && is_array($data))
		{
			...
			$this->data = array_merge($this->data, array('active_template'=>$this->active_template));
		}
	...
	public function gatherhtml($view, $data = null) {
		$thehtml = '';
		// if we have been given data, merge it
		if($data != null && is_array($data))
		{
		...
		foreach($this->templates[$this->active_template] as $temp)
		{
			...
			if($temp === '-YIELD-')
			{
				$thehtml .= $this->ci->load->view($view, $this->data, true);
			}
			else
			{
				// load the view with the class data array
				$thehtml .= $this->ci->load->view($temp, $this->data, true);
			}
